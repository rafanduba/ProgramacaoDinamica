...
void lcsLength(const std::string& X, const std::string& Y,
    ...
    // Redimensiona as tabelas para (m+1) x (n+1)
    // Usamos +1 para acomodar os casos base (i=0 ou j=0),
    // tornando os loops 1-indexados, como no livro.
    c.resize(m + 1, std::vector<int>(n + 1));
    b.resize(m + 1, std::vector<Direction>(n + 1, Direction::NONE));
